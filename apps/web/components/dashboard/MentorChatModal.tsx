...
export default function MentorChatModal({
  ...
  const fetchPeersAndUnread = useCallback(async () => {
  ...
  useEffect(() => {
    ...
    async function loadPeers() {
      ...
      setError(null);
      if (!cancelled) await fetchPeersAndUnread();
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      void fetchPeersAndUnread();
      void fetchMessages(selectedPeerId);
    }, 15000);
    return () => clearInterval(interval);
  }, [open, selectedPeerId, fetchPeersAndUnread, fetchMessages]);
      ...
      <div className="w-full max-w-3xl rounded-3xl border border-border bg-white p-6 shadow-2xl sm:p-7">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 font-display text-3xl font-bold tracking-tight text-text">Private Chat</h2>
